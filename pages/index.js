...
export default function Home() {
  const [urlMain,setUrlMain] = useState("https://pokeapi.co/api/v2/pokemon-species/?limit=21&offset=0")
  ...
  const handleSearch = async () => {
    const res = await fetch(`https://pokeapi.co/api/v2/pokemon-species/?limit=200&offset=20`);
    const data = await res.json();
        ...
        <div className="h-screen ">
            ...
            <div className="  md:flex mt-4 p-3 items-center">
             ...
             <select
              ...
              >
                ...
                {gender.map((gender,i) => (
                  <option key={i}  className="" value={gender.url}>{gender.name}</option>
                ))}
              </select>
              {/* <select id="">
              ...
              <select id="" className="" onChange={(e)=>{filterPokemonByHabitat(e.target.value)}}>
                <option value="Gender">Habitat</option>
                  {habitat.map((habitat,i) => (
                    <option key={i}  value={habitat.url}>{habitat.name}</option>
                  ))}
              </select>
             </div>
            </div>
                ...
                <div key={i}>
                   ...
                   {!pokemon.pokemon_species ? (
                    ...
                    <div
                      ...
                      className=" "
                    >
                      <Pokemon name={pokemon.pokemon_species.name} url={pokemon.pokemon_species.url} />
                    </div>
                  </Link>
                   )}
          ...
          <footer>
           {state&&(
             ...
             <button onClick={()=>{mainData.previous&&setUrlMain(mainData.previous)}} className="bg-zinc-100 hover:bg-slate-300 p-3">Prev</button>
             <button onClick={()=>{mainData.next&&setUrlMain(mainData.next)}} className="bg-zinc-100 hover:bg-slate-300 p-3">Next</button>
           </div>
           )}
          </footer>
